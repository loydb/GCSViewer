...
MUTATIONS = [
    ...
    ("print the stored refractive index in full, float noise and all",
     '        lo, hi = _ri(ri_min), _ri(ri_max)',
     '        lo, hi = str(ri_min), str(ri_max)'),
    ...
    ("lose the tier name",
     '        name = labels.get(key, "")',
     '        name = ""'),
    ("name every tier from the file instead of the P/G/C convention",
     '        name = labels.get(key, "")',
     '        name = str(grp[0].get("tier", "") or "")'),
