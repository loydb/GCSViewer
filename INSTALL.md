...
| Double-click shows **"Select an app to open this file"**, and picking the viewer does nothing | Re-run `Install-GcsViewer.ps1`. Versions 1.0.19 to 1.0.25 registered the file type without naming a default action, so Explorer had nothing to invoke: the icon was right, the default looked right in Settings, and a double-click could not proceed. Re-running repairs it. |
