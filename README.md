...
*`docs/demo.gcs`, opened in the viewer. The third panel is live: drag it to
turn the stone left and right, arrow keys to tip it up and down — that is
what the animation is doing. Sixteen pavilion mains, a girdle band, sixteen
crown mains and a table, every meet computed; generated by
[`scripts/make_demo.py`](scripts/make_demo.py) and
[`scripts/make_demo_anim.py`](scripts/make_demo_anim.py).*
...
What this adds is the part that happens outside that program: a folder full of
designs, and no way to see any of them without opening it. The viewer is for
looking, not designing — it never modifies what it opens.

GCS Viewer is an independent tool. It is not affiliated with Gem Cut Studio or
GemCad, and it reads their files the same way it reads any other.
...
`make_demo_anim.py` regenerates the animation at the top of this page, which
re-renders only the 3/4 panel per frame and assembles each one through the
viewer's own `compose()`, so the layout cannot drift from what the program
produces.
