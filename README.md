...
diagrams. That makes them a shared surface with a real dependent, so changes
here are additive: new optional parameters, never a changed signature or a
changed meaning. The parser checks in `test_gcs_viewer.py` are the contract.
